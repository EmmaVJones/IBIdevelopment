...
Break into chunks to ease processing. Using assessement regions

```{r streamstats delineation}

needWatershedFilter <- filter(needWatershed, ASSESS_REG == 'BRRO')
...
if(nrow(points) != nrow(watersheds) | nrow(needWatershed) != nrow(watersheds)){
  missing <- unique(
    c(as.character(points$UID[!(points$UID %in% watersheds$UID)]),
      as.character(needWatershed$StationID[!(needWatershed$StationID %in% watersheds$UID)])))
  missingDat <- filter(needWatershed, StationID %in% missing)
