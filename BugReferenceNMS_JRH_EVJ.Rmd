...
#setwd('C:/Users/ktq89598/Desktop/FishBugIBI/NMDS')

```
## Methods


<font size="4"> The document serves as an example of analyses that will be conducted to identify natural variations in benthic communities across Virginia. These NMDS will support the Genus level IBI development process. An analysis is the first run of all of reference sites in Virginia. No West Virginia DEP data is used in this analysis. Several reference sites have been sent back to Virginia DEQ biologists for one more review. 
...
###Load Data
bugs <- read.csv('data/dataOut/benthics.csv')
env <- read.csv('data/dataOut/stationsSpatial_withAllData.csv')
Bugenv<-left_join(bugs,env, by="BenSampID")
# bugs <- read_excel('JRH_Data/BugsJune11.xlsx', sheet='RefBugV1')
# 
# env <- read_excel('JRH_Data/BugsJune11.xlsx', sheet='EnvV1')
...
######think lucy was log10 bugs here?####


bugs[is.na(bugs)]<-0
bugsnms<-bugs%>%
select("StationID", 6:115) # not best practices to call column numbers

buglog<-log1p(bugsnms[,2:111])# not best practices to call column numbers

###buglog$site<-1:nrow(buglog) 
###bugsnms$site<-1:nrow(bugsnms)


#######It this where lucy is putting species fit into graph??
#Site<-bugsnms%>%
...
Number of rows in Community Matrix: `r format(nrow(bugs))`

Number or rows in Environmental Matrix: `r format(nrow(env))`
...
```{r bugnms, echo=FALSE}
NMDSbug <-metaMDS (bugs[,8:131],k=3) 
#NMDSbug <-metaMDS (bugs[,8:131],k=3) 
#Best run is 0.18
NMDSbug
...
legend("topright", legend = c(levels(env$Season)),pch = c(16, 8, 16, 16))
legend("topleft", "stress = 0.181", bty = "n", cex = 2) # displays legend text of stress value 
#plot(bug.spp.fit, p.max = 0.001, col = "black", cex = 1) 

#head(bug.spp.fit$vectors)


#plot (all) with species by Season
#with(env, levels(Season))
#scl <- 2 ## scaling = 2
#colvec <- c("red2", "mediumblue")
#plot(NMDSbug, type = "n", scaling = scl)
#with(env, points(NMDSbug, display = "sites", col = colvec[Season],
#                  scaling = scl, pch = 21, bg = colvec[Season]))
#with(env, legend("topright", legend = levels(Season), bty = "n",
