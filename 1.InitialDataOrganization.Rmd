...
stationsSpatial2 <- left_join(stationsSpatial, bind_rows(prob, BCG), by = 'StationID') %>% 
...
benSamps <- pin_get('ejones/benSamps', board = 'rsconnect') %>% 
  filter(StationID %in% stationsSpatial2$StationID) %>% 
...
stationsSpatial_withAllData <- left_join(
  dplyr::select(benSamps, StationID, `Collection Date`, Gradient, Season, JulianDate),
  stationsSpatial2, by = 'StationID') %>% 
