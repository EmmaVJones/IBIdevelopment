  ...
  if( nrow(b) == 0 ){
    ...
    b <- mutate(b,`Point Unique Identifier` = cn,
                `Buffer Distance` = paste(bufferDistances[x], 
                                          st_crs(POINT)$units, sep = ' ')) %>%
      dplyr::select(`Point Unique Identifier`, `Buffer Distance`, !! MULTILINESTRING_colname) %>%
      mutate_all(as.character) %>% # just to be safe, make everythign character
      st_drop_geometry()}
  return(b)
}
...
snap_Points_to_Feature <- function(MULTIPOINT, # sf MULTIPOINT file
  ...
  zzz <- sym(MULTILINESTRING_colname)
  
  # place to store stuff
  tbl_output <- tibble(`Point Unique Identifier`= character(), `Buffer Distance` = character(), outField = character()) %>% 
      rename( !! zzz := 'outField')
  ...
  for(i in 1:nrow(MULTIPOINT)) {
    ...
    z <- snap_Point_to_Feature(MULTIPOINT[i,], POINT_UID_colname, MULTILINESTRING, MULTILINESTRING_colname, bufferDistances)
...
#snap_Points_to_Feature(sites, POINT_UID_colname, MULTILINESTRING, 'STRAHLER' ,bufferDistances)


snapAndOrganizeAnything <- function(sites, # sf MULTIPOINT file
                               POINT_UID_colname,# as.character(name of unique identifier in POINT file)
                               MULTILINESTRING,# stream network
                               MULTILINESTRING_colname, # name of field to be taken from network
                               bufferDistances, # numeric sequence of distances to run buffer, these will be in
  ...
  # 6/14/21 can use epsg code again
  ...
  MULTILINESTRING <- MULTILINESTRING %>% st_transform(102003) 
  # 6/14/21 can use epsg code again
  #  st_transform('+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m no_defs')
  # 2/3/21 have to manually feed character proj4string bc doesn't recognize albers epsg 
  ...
  snapList <- snap_Points_to_Feature(sites, POINT_UID_colname,
                                         MULTILINESTRING, MULTILINESTRING_colname, 
                                         bufferDistances)
  outTable <- bind_rows(outTable,
                        rename(snapList, !! sym(POINT_UID_colname) := 'Point Unique Identifier') %>%
                          dplyr::select( !! POINT_UID_colname, !! MULTILINESTRING_colname, `Buffer Distance`)) %>% 
    filter(!is.na( !! sym(POINT_UID_colname) ))
